export function hashPassword(str: string): string {
  ...
  for (let i = 0; i < saltedStr.length; i++) {
    const code = saltedStr.charCodeAt(i);
    if (code < 128) {
      bytes.push(code);
    } else if (code < 2048) {
  ...
  for (let i = 0; i < 8; i++) {
    const shift = (7 - i) * 8;
    bytes.push((shift < 32 ? bitLength >>> shift : 0) & 0xff);
  }
  ...
  const hexIdx = '0123456789abcdef';
  let resultHex = '';
  const hashArr = [h0, h1, h2, h3, h4, h5, h6, h7];
  for (let i = 0; i < hashArr.length; i++) {
    const val = hashArr[i];
    for (let j = 7; j >= 0; j--) {
      const byteVal = (val >>> (j * 4)) & 0xf;
      resultHex += hexIdx[byteVal];
    }
  }
  return resultHex;
}
